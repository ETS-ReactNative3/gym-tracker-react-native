...
class AddExerciseList extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      ...
      .then(response => {
        console.log("filtered array to be set: ", response)
        this.setState({
            exerciseList: response,
            filteredArray: response
          }, () => console.log("filtered array updated in component did mount: ", this.state.filteredArray))
      })
  ...
  componentWillUnmount() {
    //   pass array of exercises to add to redux action
        ...
        <ScrollView>
          ...
          <View>
            {this.state.filteredArray.map(exercise => {
                console.log("RENDER - ex obj: ", exercise, "Exercise Name: ", exercise.exerciseName)
              return (
                <ExerciseListItem
                  id={exercise.id}
                  key={Math.random()}
                  exerciseName={exercise.exerciseName}
                  onPress={() => this.toggleHighlightItem(exercise.name)}
                  buttonSelected={
                    this.state.itemIdList.includes(exercise.name)
                      ? style.highlighted
                      : style.unHighlighted
                  }
                />
